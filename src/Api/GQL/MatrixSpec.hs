...
-- Portability : POSIX
--
module Api.GQL.MatrixSpec where
---------------------------------------------------------------------------------
...
resolverMatrixSpec req@Model.Request {..} = do
  ...
  pure $ GqlType.MatrixSpec { subExpression  = resolverSubExp subReq
...
resolverSubExp mix = do
  ...
  pure $ GqlType.Expression { source  = resolverSource (Model.subjectType mix)
 ...
 where
  ...
  mkFields = traverse identity (traverse identity . fmap mkName <$> fields)
  ...
  mkFilter = traverse
    (\vs -> pure $ GqlType.Filter { relations = mkRelations vs
                                  , fieldName = pure Nothing
                                  }
    )

  mkRelations
    :: GraphQL o m => Model.ReqQualities -> ArrayObject o m GqlType.Relation
  mkRelations reqQuals = traverse identity (fromReqQualities go reqQuals)
   ...
   where -- compose fst . resolverRelation
    ...
    go key vs =
      let (vs', antiRequest') = Model.unwrapReqEnum vs -- ✨ From type tag to value
      in  fst $ resolverRelation key (Values vs' antiRequest')
...
--
--    🔖 The UI can workaround this for MVP
--
resolverMeaExps
  :: (GraphQL o m, MonadLogger m)
...
-- > [[Relation]] -> [[Relation]]         :: cartesianProduct
--
-- > [Relation] -> Filter
-- > Filter   -> Exp           1:1
--
resolverComponentMix
  ...
  -> [Object o m GqlType.Expression]
resolverComponentMix mix = case snd mix of
  Just vs -> mkExps (fst mix) (mkFilters vs)
  Nothing ->
    [ pure $ GqlType.Expression { source  = resolverSource (fst mix)
                                , filter  = pure Nothing
                                , fields  = pure field
   ...
   where
    field =
      traverse identity [pure $ GqlType.FieldName (pure $ nameTag (fst mix))]
 ...
 where
  ...
  mkFilter relations' = pure $ GqlType.Filter
    { relations = traverse identity (fst <$> relations')
    , fieldName = (pure . Just . GqlType.FieldName . pure)
                  . intercalate "."
                  $ nameTag (fst mix)
                  : (snd <$> relations')
    }
  ...
  mkExps src filters =
    fmap
        (\flt -> GqlType.Expression { source  = resolverSource src
      ...
      <$> filters
   where
    field = traverse identity [pure $ GqlType.FieldName (pure $ nameTag src)]
...
--
-- There is a second qualifier: 'Model.FieldValues.AntiRequestEnum'. Read more
-- about the interaction of the two qualifiers.
--
--
-- 🚧 ⚠️   A request with a 'Model.ETL.FieldValues.AntiRequestEnum' value of
--        @Exclude@ is ignored when the 'Model.ETL.TagRedExp' is set to @Exp@.
--
...
interpretExpRed fldName vs
  | Model.areSpanValues vs
  = uncurry resolverRelation
    <$>   repeat fldName
    `zip` (Range <$> concat (Values.mkFilterRange <$> toList vs))
  |
       ...
       --
       -- 🚧 ⬜ Initial view: any Exp request ignores Include/Exclude tag
       --       using Include.
       --
    Model.isExp vs
  = uncurry resolverRelation
    <$> repeat fldName
    `zip` ( flip Values Model.Include <$> Model.toCompValuesList vs)
  |
       --
       -- CompReqValues Exp FieldValues fromList ["cash", "medicare"]
       -- Relation payer EQ ["cash"]
       -- Relation payer EQ ["medicare"]
       -- 🚧 ⬜ Be prepared to update impact of antiRequest (or not)
       --
    Model.isRed vs
  = let Model.CompReqValues tvs = vs
    in  [resolverRelation fldName (uncurry Values . Model.unwrapReqEnum $ Tag.unTag tvs)]
  |
        --
        ...
        --
    otherwise
  = panic "interpretExpRed should not have reached here"


---------------------------------------------------------------------------------
-- |
-- Internal unifying type structure to host the RHS of
-- 'resolverRelation' input
--
-- 🚧 Hosts the antiPatter value
--
data RelationInputValues
  = Values { values ::  Values.FieldValues, antiRequest :: Model.AntiRequestEnum }
...
getOtherValues (Values vs _) = vs
getOtherValues _             = panic "getOtherValues: Unreachable"

isExcludeRequest :: RelationInputValues -> Bool
isExcludeRequest (Values _ Model.Exclude) = True
isExcludeRequest _                        = False
...
instance NameTagC RelationInputValues where
  nameTag (Values vs _) = nameTag vs
...
resolverRelation key vs' =
  ( pure $ GqlType.Relation { lhs      = resolverLHS key
 ...
 where
     ...
     -- ⬜ refactor to improve transparency of the logic
     --
  resolverRelationSymbol vs
    |
  -- Process range -> BETWEEN | NOT_BETWEEN
      isRange vs && isExcludeRequest vs = if len (getRangeValue vs) > 1
      then pure GqlType.NOT_BETWEEN
      else pure GqlType.NOT_EQ
    |
  --
    ...
    |
      --
  -- Process request for a single value (non-range) -> EQ | NOT_EQ
      len (getOtherValues vs) == 1 && isExcludeRequest vs = pure GqlType.NOT_EQ
    | len (getOtherValues vs) == 1 = pure GqlType.EQ
    |
    --
  -- Process request for a more than one value (non-range) -> IN | NOT_IN
      isExcludeRequest vs = pure GqlType.NOT_IN
  ...
  resolverLHS =
    pure . GqlType.LHS . pure . GqlType.FieldName . pure . Model.unKey
  ...
  resolverRHS values
    | not (isRange values) = case values of
      ...
      (Values (Model.TxtSet vs) _) ->
        GqlType.RHSTxtValues <$> Shared.resolverTxtValues (Model.TxtSet vs)

      (Values (Model.IntSet vs) _) ->
        GqlType.RHSIntValues <$> Shared.resolverIntValues (Model.IntSet vs)
...
--
resolverSpanFilter
  :: GraphQL o m => Values.FilterRange -> Object o m GqlType.SpanFilter

resolverSpanFilter Values.FilterRange {..} =
  pure . GqlType.SpanFilter . pure $ GqlType.FilterRange (pure filterStart)
                                                         (pure filterEnd)
